...
class MainWindow(QtWidgets.QMainWindow, QObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print('Start Time:', self.timestamp[0])
    ...
    def update_plot_data(self, task):
        ...
        preset_interval = 60
        print ('Receiving', task )
        self.win_data = self.hr [(self.timestamp >= task - preset_interval ) & (self.timestamp < task + preset_interval)]
        self.win_time = self.timestamp[(self.timestamp >= task - preset_interval) & (self.timestamp < task + preset_interval)]
